...
def save_images(original_images, watermarked_images,noised_images, epoch, folder, resize_to):#resize_to=None
    ...
    if resize_to is not None:
        images = images.float()  # 转换为 float32i
        watermarked_images = watermarked_images.float()  # 转换为 float32
...
def save_checkpoint(model:ROIF, experiment_name: str, epoch: int, checkpoint_folder: str):
...
def model_from_checkpoint(ROIF, checkpoint):

    ROIF.encoder_decoder.load_state_dict(checkpoint['enc-dec-model'])
    ROIF.optimizer_enc_dec.load_state_dict(checkpoint['enc-dec-optim'])
    ROIF.discriminator.load_state_dict(checkpoint['discrim-model'])
    ROIF.optimizer_discrim.load_state_dict(checkpoint['discrim-optim'])
